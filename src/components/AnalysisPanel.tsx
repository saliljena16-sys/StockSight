...
export default function AnalysisPanel({ analysis, ticker }: AnalysisPanelProps) {
  ...
  return (
    <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-6">
              ...
              <div className="w-24 h-2 bg-slate-700 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full ${
                    analysis.confidence > 70 ? 'bg-emerald-500' : analysis.confidence > 40 ? 'bg-amber-500' : 'bg-red-500'
                  }`}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
        ...
        <div className="bg-slate-900/50 rounded-lg p-3 border border-slate-700/30">
          ...
          <p className={`text-lg font-bold ${
            analysis.currentRSI > 70 ? 'text-red-400' : analysis.currentRSI < 30 ? 'text-emerald-400' : 'text-white'
          }`}>
      ...
      <div>
        <h3 className="text-sm font-semibold text-slate-300 mb-3">Signals for {ticker}</h3>
          ...
          {analysis.signals.map((signal, index) => (
            ...
            >
              <div className={`mt-0.5 ${
                signal.type === 'bullish' ? 'text-emerald-400' : signal.type === 'bearish' ? 'text-red-400' : 'text-slate-400'
              }`}>
                {signal.type === 'bullish' ? (
              ...
              <div>
                <p className={`text-sm font-medium ${
                  signal.type === 'bullish' ? 'text-emerald-300' : signal.type === 'bearish' ? 'text-red-300' : 'text-slate-300'
                }`}>
                  {signal.name}
                </p>
