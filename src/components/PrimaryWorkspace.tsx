  ...
  return <>
    ...
    <div className="grid gap-4 lg:grid-cols-[1.35fr_.85fr]">
      <Panel className="overflow-hidden">
        ...
        <div className="divide-y divide-white/[.06]">{ranked.map((stock, index) => { const move = changePercent(stock); return <button key={stock.ticker} onClick={() => onInspect(stock.ticker)} className="grid w-full grid-cols-[28px_1fr_auto_auto] items-center gap-3 px-5 py-3 text-left hover:bg-white/[.035]"><span className="font-mono text-xs text-[var(--text-tertiary)]">{String(index + 1).padStart(2, '0')}</span><span className="flex items-center gap-3"><CompanyMark ticker={stock.ticker} size={28} /><span><span className="block text-sm font-semibold text-white">{stock.ticker}</span><span className="block text-[11px] text-[var(--text-tertiary)]">{stock.sector}</span></span></span><span className="font-mono text-sm text-white">${stock.currentPrice.toFixed(2)}</span><span className={`min-w-20 text-right font-mono text-sm ${move >= 0 ? 'text-[var(--positive)]' : 'text-[var(--negative)]'}`}>{move >= 0 ? '+' : ''}{move.toFixed(2)}%</span></button>})}</div>
      </Panel>
